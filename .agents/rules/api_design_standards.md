...
- **Kebab-Case URLs**: Nutze `api/v1/patienten-daten` statt
  `api/v1/PatientenDaten`.
...
- **Erfolg**: `200 OK`, `201 Created` (nach POST), `204 No Content` (nach
  DELETE/PUT ohne Body).
- **Fehler**: `400 Bad Request` (Validierung), `401 Unauthorized`,
  `403 Forbidden`, `404 Not Found`, `500 Internal Server Error`.
- **Besonderheit**: Nutze standardisierte Response-Objekte für Fehlermeldungen
  (z.B. RFC 7807 Problem Details).

## 🛡️ Best Practices

- **Versionierung**: Nutze URL-Versionierung (z.B. `/api/v1/...`).
- **DTOs**: Nutze DTOs für Input und Output. Gib NIEMALS Domain-Entities direkt
  zurück.
